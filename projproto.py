...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.ui.addWalletButton.clicked.connect(self.add_wallet)
        self.ui.deleteWalletButton.clicked.connect(self.delete_wallet)
        self.ui.addExpenseButton.clicked.connect(self.add_expense)
        self.ui.addIncomeButton.clicked.connect(self.add_income)
        self.ui.transferButton.clicked.connect(self.transfer_money)
        self.ui.removeAccountButton.clicked.connect(self.remove_account)
 ...
 "font: 10pt 'MS Shell Dlg 2';}")
    ...
    password TEXT NOT NULL);'''
        query2="CREATE TABLE Wallet (wallet_id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,wallet_name TEXT NOT NULL,balance REAL NOT NULL DEFAULT 0,FOREIGN KEY (user_id) REFERENCES User(user_id));"
    ...
    FOREIGN KEY (wallet_id) REFERENCES Wallet(wallet_id));'''
      ...
      def login(self,password,username):
				result=self.cursor.execute('select user_id from User where password=? and username=?;',(password,username));
        if result!=None:
						return 	FALSE
        else
        		return TRUE
